...
{include:top-products} <?php if ( !empty($products) ) : ?>

<div id="tri-price"><?php printf( __('Sort by price <a href="%1$s">+</a>/<a href="%2$s">-</a>', 'adfever'), get_adfever_link('category-order', 'more'), get_adfever_link('category-order', 'less') ); ?>
</div>
<table id="list-products">
	...
	?>
		<tr>
			...
			<td style="text-align: center;" width="20%"><?php _e('Best price', 'adfever'); ?>
			<br />
			<span class="product-price"><?php echo number_format($product['prix_moins']['value'], 2, ',', ' ').$product['prix_moins']['currency']; ?></span>
			<br />
			<em><?php printf( __('At <a href="%1$s">%2$s</a>', 'adfever'), clean_url(get_adfever_link('product', $product['id'])), wp_specialchars($store['merchant']) ); ?></em>
			<br />
...
<p class="clear"><?php _e('No products for this category.', 'adfever'); ?></p>
